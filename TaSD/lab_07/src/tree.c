...
int tree_read(tree_t **tree, char *file_name)
{
    ...
    while (rc == ERR_OK)
    {
        tree_t *node = tree_node_create("", "");
        if (node == NULL) rc = ERR_MEM;
        if (!rc) rc = tree_node_read(f, NULL, node);
...
const char *tree_find(tree_t *tree, const char *name)
{
    if (tree == NULL)
        return NULL;
    const char *found_char = NULL;
    while (tree != NULL && found_char == NULL)
    {
        int rc = strcmp(name, tree->name);
        if (rc == 0)
            found_char = tree->manual;
        else if (rc < 0)
            tree = tree->left;
        else
            tree = tree->right;
...
size_t tree_get_size(tree_t *tree)  
{
    ...
    if (tree->name != NULL) s += sizeof(char) * strlen(tree->name);
    if (tree->manual != NULL) s += sizeof(char) * strlen(tree->manual);
    
    s += tree_get_size(tree->left);
    s += tree_get_size(tree->right);
    return s;
}
