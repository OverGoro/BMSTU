...
SplitterModel::SplitterModel(QObject *parent)
    : QAbstractTableModel{parent}
{}

void SplitterModel::setFigures(const std::vector<Figure> &figures)
{
    this->figures = figures;
}

std::vector<Figure> SplitterModel::getOpenedFigures()
{
...
std::vector<Figure> SplitterModel::getClosedFigures()
{
...
void SplitterModel::addPoint(QPoint p)
{
    if (figures.size() == 0)
    {
...
void SplitterModel::closeFigure()
{
    if (figures.size() == 0)
...
void SplitterModel::clear()
{
    figures.clear();
    emit layoutChanged();
}

void SplitterModel::pop()
{
    if (figures.size() == 0)
        return;
...
int SplitterModel::rowCount() const
{
    int rows_num = 0;
    for (auto f : figures)
    {
...
int SplitterModel::rowCount(const QModelIndex &parent) const
{
    return rowCount();
}

int SplitterModel::columnCount() const
{
    return 2;
}

int SplitterModel::columnCount(const QModelIndex &parent) const
{
    return columnCount();
}

QVariant SplitterModel::data(const QModelIndex &index, int role) const
{
