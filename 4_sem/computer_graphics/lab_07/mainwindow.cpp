...
void MainWindow::painter_widget_clicked(QMouseEvent *event)
{
    ...
    {
        ui->x1_lineEdit->setText(QString::number(event->pos().x()));
        ui->y1_lineEdit->setText(QString::number(event->pos().y()));
        ui->painter_widget->repaint();
    }
    else if (event->button() == Qt::MiddleButton)
    {
        ui->x2_lineEdit->setText(QString::number(event->pos().x()));
        ui->y2_lineEdit->setText(QString::number(event->pos().y()));
        ui->painter_widget->repaint();
    }
}
...
void MainWindow::draw_clipper()
{
    ...
    ui->painter_widget->drawClipper(x1, y1, x2, y2, clipperColor);
}
...
void MainWindow::draw_clipped()
{
    ...
    for (auto l : lines)
    {
        try
        {
            ui->painter_widget->drawClippedLine(x1, y1, x2, y2, l, clippedColor);
        }
        catch (std::exception e)
        {
